...
def analyze_function(bv, name, var_memory, var_size):
    ...
    for ref in bv.get_code_refs(function_sym.address):
        ...
        if (source.value.type == RegisterValueType.UndeterminedValue and  
            size.value.type == RegisterValueType.ConstantValue):
            
            source_ssa_var = source.src
            var = SearchVar(bv, function.start, source_ssa_var)
            var.search_var_initialization()

            if not var.target_var: 
                continue
            
            for ssa_var, value in var.target_var.items():
                
                if isinstance(value, int):
                    dominators = get_mlil_dominators(function, mlil)
                    tracer = MLILTracer(bv, function.start, dominators, {})
                    tracer.set_function_vars({ssa_var:value})
                    tracer.trace()
                    leaked_offsets = tracer.check_info_leak(value, size.constant)
                    del tracer
                    ...
                    tracer.trace()
                    leaked_offsets = tracer.check_info_leak(value.offset, size.constant)
                    del tracer
